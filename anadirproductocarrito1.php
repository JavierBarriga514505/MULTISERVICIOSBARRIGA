<?php
//Este es un proceso de recibir datos por medio de post y la incersion de los mismo en el carrito de compras de un usuario en especifico
...
if (!empty($_POST['stock_producto']) && !empty($_POST['precio_producto']) && !empty($_POST['id_producto']) && !empty($_POST['codigo_producto'])  && !empty($_POST['cantidad_pedir'])) {
	...
	if ($query_check_user >= 1) {
		...
		if ($query_check_user <= 0) {
			...
			if ($query) {
				echo "<script>alert('Producto Añadido al Carrito.');location.href ='historial_venta.php';</script>";
			}
		} else {
