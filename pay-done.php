  ...
  if ($response->{"data"}->{"x_test_request"} == "TRUE") {
    ...
    switch ((int) $response->{"data"}->{"x_cod_transaction_state"}) {
      case 1:
        ...
        $sqlStatePay = $conn->query("UPDATE compras SET estado='2' WHERE id_compra='" . $response->{"data"}->{"x_id_invoice"} . "';");
      ...
      case 12:
        ...
        $sqlStatePay = $conn->query("UPDATE compras SET estado='1' WHERE id_compra='" . $response->{"data"}->{"x_id_invoice"} . "';");
        if ($sqlStatePay) echo "<script>console.log('Actualización de estado de PAGO exitosa.');</script>";
        else echo '<script>console.log("Error en estado de PAGO: ' . $conn->error . '.");</script>';
        break;
      case 3:
...
?>
  ...
  <div class="main-bg">
    <!-- title -->
    <h1>MULTISERVICIO BARRIGA<br>[Transacciones]</h1>
    <!-- //title -->
      ...
      <div class="row col-12 col-lg-9 vertical-tab">
        <div id="section-finish" class="section">
          ...
          <article class="col-12 col-lg-9 form">
            ...
            <a href="ver_compras.php" class="btn submit">TERMINAR</a>
          </article>
        </div>
      </div>
      <!-- //vertical tabs -->
