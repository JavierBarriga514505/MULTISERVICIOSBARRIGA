<?php 
// en esta seccion se reciben todos los datos enviado mediante POST para registrar la compra de los productos del carrito de un usuario en especifico por parte del administrador
...
if (!empty($_POST['id'])) {
	...
	if ($variable = 1) {
		...
		while ($row = mysqli_fetch_array($query)) {
			...
			if ($envio == 'SI') {
				$insertar = "INSERT INTO envios(id_compra,name,tipo_documento,numero_documento,celular_usuario,email,direccion_usuario,ciudad_usuario) VALUES('$totalregistro','$name','$tipo_documento','$identificacion','$celular','$email','$direccion','$ciudad');";
			...
			while ($row0 = mysqli_fetch_array($query0)) {
				...
				if ($cantidad_producto > $stock) {
					...
					$sql2 = "INSERT INTO compras(id_usuario,total,fecha,estado,foto_recibo) VALUES('$id_usuario','$totalsuma',CURTIME(),'1','uploads/noimage.jpeg');";
		...
		if ($query2 && $query3) {
			echo "<script>alert('Se ha generado la transaccion correctamente.');window.location.href = 'factura.php?mensaje='+encodeURIComponent($totalregistro)+'&usuario='+encodeURIComponent($id_usuario);</script>";
			unset($variable);
			$variable = 0;
		} else {
		...
		echo "<script>alert('Ya se ha hecho la transaccion.');location.href ='carrito.php';</script>";
	}
} else {
